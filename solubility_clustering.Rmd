...
```{r}
solubility_vs_mw <- df %>%
  select(solubility, mw) %>%
  as.matrix() %>%
  scale()

solubility_vs_ml_dist <- dist(solubility_vs_mw)
solubility_vs_ml_hclust <- hclust(solubility_vs_ml_dist, method = "complete")
plot(solubility_vs_ml_hclust)
```

```{r}
alpha <- 0.25
solubility_vs_ml_cluster <- as.factor(cutree(solubility_vs_ml_hclust, k = 4))
df %>%
  transmute(
    solubility,
    mw,
    cluster = solubility_vs_ml_cluster
  ) %>%
  ggplot(aes(x = mw, y = solubility, col = cluster)) +
    geom_jitter(alpha = alpha)
```

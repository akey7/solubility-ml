...
Now, try a model with all possible predictors

``` {r}
everything_model <- lm(solubility ~ mw + h_bond_donors + rings + rotatable_bonds + psa + minimum_degree, train)
everything_result <- analyzeSolubilityModel(everything_model, test)
```

Now, try to reduce the number of features

``` {r}
limited_model <- lm(solubility ~ mw + h_bond_donors, train)
limited_result <- analyzeSolubilityModel(limited_model, test)
```

...
## Random dforest failure analysis

Since the random forest fared better, let's analyze what compounds make it fail. Are there any structural commonalities among the compounds that caused over or underpredictions? I will define underprediction as a residual > 1.0 and overprediction as a residual < 1.0. Remember that this is a log solubility scale, so these thresholds correspond to more than an order of magnitude difference in actual versus predicted solubility.


Table 6 contains the mean values of molecular properties for the entire test compounds, the solubility-overpredicted compounds, and the solubility-underpredicted compounds. A caveat with these values is that h_bond_donors, rings, and rotatable_bonds must be integers greater than or equal to zero, so fractional values for these properties make no physical sense. However, fractional values can indicate trends in the data that are interesting.

##### Table 6: Mean values of molecular properties

``` {r}
rf_overpredictions <- rf_result$test_results %>%
  filter(residual < -1.0)

rf_underpredictions <- rf_result$test_results %>%
  filter(residual > 1.0)

train_summary <- train %>%
  select(!c("compound", "solubility")) %>%
  pivot_longer(everything(), names_to = "feature", values_to = "test_mean") %>%
  group_by(feature) %>%
  summarize_all(mean)

rf_overpredictions_summary <- rf_overpredictions %>%
  select(!c("compound", "residual", "prediction", "solubility")) %>%
  pivot_longer(everything(), names_to = "feature", values_to = "overpredicted_mean") %>%
  group_by(feature) %>%
  summarize_all(mean)

rf_underpredictions_summary <- rf_underpredictions %>%
  select(!c("compound", "residual", "prediction", "solubility")) %>%
  pivot_longer(everything(), names_to = "feature", values_to = "underpredicted_mean") %>%
  group_by(feature) %>%
  summarize_all(mean)

join_summaries <- train_summary %>%
  inner_join(rf_overpredictions_summary, by = "feature") %>%
  inner_join(rf_underpredictions_summary, by = "feature")

knitr::kable(join_summaries)
```


Of those trends, I observe that for compounds with underpredicted solubilities, the number of h_bond_donors is higher, which leads me to think the model does not place proper importance on hydrogen bonding for aqueous solubility. For compounds with overpredicted solubilities, the molecular weight is higher, and there are more rotatable bonds, which leads me to believe the model is confused by large, flexible molecules. Perhaps these observations could provide clues as to future feature engineering possibilities.

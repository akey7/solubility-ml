...
The second model is the random forest model. For the random forest, [I use the ranger package](https://www.rdocumentation.org/packages/ranger/versions/0.12.1/topics/ranger). The model uses 500 trees and is trained and tested with the same tran and test data as the original linear model. Table 4 shows the random forest's prediction performance on the test data after training.

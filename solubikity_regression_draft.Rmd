...
[Caffiene](https://en.wikipedia.org/wiki/Caffeine) | I drank too much of it right now...
...
The first few rows of the train dataset:
...
And the first few rows of the test dataset:
...
lm_result_df <- data_frame(
  metric = c("sd(test$solubility)", "RMSE", "R^2"),
  value = c(sd(test$solubility), lm_result$rmse, lm_result$r_squared)
)
...
The second model is the random forest model. For the random forest, I use the ranger package. The model uses 500 trees and is trained and tested with the same tran and test data as the original linear model. Table 4 shows the random forest's prediction performance on the test data after training.
...
rf_result_df = data_frame(
  metric = c("sd(test$solubility)", "RMSE", "R^2"),
  value = c(sd(test$solubility), rf_result$rmse, rf_result$r_squared)
)

knitr::kable(rf_result_df)
```
The performance of the random forest model is substantially better than the linear model. The RMSE of 0.861 is less than half the solubility's standard deviation of 2.09. The random forest explains much more of the variability of the solubiltiy with an R^2 value of 0.830. Anecdotally, in this configuration and on this dataset, the random forest execution time is similar to that of the linear model.